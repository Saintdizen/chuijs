...
class TgTestPage extends Page {
    ...
    constructor() {
        ...
        let bot_chat_data = TgTestPage.#fieldSet({
            title: "Данные чата и бота",
            components: [this.#chat_id, this.#bot_token],
        });
        this.add(bot_chat_data);
        // Отправка сообщения
        let field_send_message = TgTestPage.#fieldSet({
            title: "Отправка сообщения",
            components: this.#send_m(),
        });
    ...
    #chat_settings() {
        ...
        contentBlock.add(title, button_1, description, button_2, fileInput, button_3, button_4);
        return contentBlock;
    }
    #gets(name) {
        return new Button({
