const { Main, MenuItem, path, App, formatBytes, Log, systemPreferences } = require("./index");
            ...
            item.on("updated", (event, state) => {
                if (state === "interrupted") {
                    ...
                    if (item.isPaused()) {
                        ...
                        console.log(
                            `Received bytes: ${formatBytes(item.getReceivedBytes())} - ${formatBytes(item.getTotalBytes())}`
                        );
                    }
                }
            });
