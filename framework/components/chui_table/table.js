...
class Table {
    ...
    constructor(
        ...
        if (!this.#data || this.#data.length === 0) {
            throw new Error("Table: массив data пуст — передайте хотя бы одну строку для определения колонок");
        }
        //======================
    ...
    setContextMenu(ctx = new HTMLElement()) {
        if (globalThis.ctxs === undefined) globalThis.ctxs = [];
        globalThis.ctxs.push({ elem: this.#table, ctx: ctx });
    }
    ...
    setFilterByProperty(type = String(), property = String(), filterValue = String()) {
        ...
        if (type.includes(Table.FILTER_TYPE.CLEAR_MATCH)) {
            if (this.#filtered_data.length !== 0) {
                this.#filtered_data = this.#filtered_data.filter(
                    (data_1) => String(data_1[property]).toLowerCase() === String(filterValue).toLowerCase()
                );
            } else {
        ...
        if (type.includes(Table.FILTER_TYPE.PARTIAL_MATCH)) {
            if (this.#filtered_data.length !== 0) {
                this.#filtered_data = this.#filtered_data.filter((data_1) =>
                    String(data_1[property]).toLowerCase().includes(String(filterValue).toLowerCase())
                );
            } else {
                this.#filtered_data = this.#data.filter((data_1) =>
                    String(data_1[property]).toLowerCase().includes(String(filterValue).toLowerCase())
                );
            }
        }
    ...
    removeFilterByProperty(property = String(), filterValue = String()) {
        this.#data
            .filter((data_1) => String(data_1[property]).toLowerCase() === String(filterValue).toLowerCase())
            .forEach((data_2) => {
    ...
    #setWidthColumns(row) {
        if (this.#columnsWidth === undefined) {
            let arr = [];
            for (let i = 0; i < this.#columns.length; i++) arr.push(`calc(100% / ${this.#columns.length})`);
    ...
    #setHeaders() {
        ...
        if (this.#customNames !== undefined) {
            cols = this.#customNames;
        }
