...
class Audio {
    ...
    constructor(
        options = { autoplay: Boolean(), pin: String(), playlist: Boolean(), width: String(), height: String() }
    ) {
        ...
        // КНОПКИ
        this.#chui_ap_play_pause.innerHTML = new Icon(
            Icons.AUDIO_VIDEO.PLAY_ARROW,
            this.#icons_sizes.play_pause
        ).getHTML();
        ...
        this.#chui_ap_volume.addEventListener("input", () => {
            ...
            if (Number(this.#chui_ap_volume.value) === 0) {
                this.#chui_ap_volume_icon.innerHTML = new Icon(
                    Icons.AUDIO_VIDEO.VOLUME_MUTE,
                    this.#icons_sizes.volume
                ).getHTML();
            } else {
                this.#chui_ap_volume_icon.innerHTML = new Icon(
                    Icons.AUDIO_VIDEO.VOLUME_UP,
                    this.#icons_sizes.volume
                ).getHTML();
            }
        });
    ...
    setPlayList(list = [{ title: String(), artist: String(), album: String(), mimetype: String(), artwork: [] }]) {
        ...
        for (let track of play_list)
            this.#chui_playlist.getPlaylist().appendChild(this.#setTrack(track, play_list.indexOf(track)));
    }
...
class AudioFX {
    ...
    constructor(audio) {
        ...
        this.#select.addValueChangeListener((e) =>
            this.#filters.forEach((filter) => this.#setPreset(filter, e.detail.value, true))
        );
        //
    ...
    #setSliderPreamp(filters) {
        ...
        slider.oninput = (e) =>
            AudioFX.#renderPreampSlider(e.target, this.#getPreset(store.get(this.#fx_preset)), filters);
        val.id = "val_preamp";
    ...
    #setSliderBand(filter) {
        ...
        slider.style.setProperty("--fx-before-width", `50%`);
        slider.oninput = (e) =>
            AudioFX.#renderSlider(e.target, filter, Number(document.getElementById("preamp").value));
